...
import useUser, { defaultAvatar, AvatarErrorLoad, type Myself } from '../lib/user';
...
export default function Profile ()
{
    ...
    const [selectUser, setSelectUser] = useState< Myself | null>(null);
    ...
    const [getSearchVal, setGetSeachVal] = useState<Myself[]>([]);
    ...
    const matchWins = (selectUser?.matchWins ?? [])?.map((m) => ({
        ...m,
        result: t('profile.win'),
        opponent: m.loserLabel
    }));

    const matchLosses = (selectUser?.matchLosses ?? [])?.map((m) => ({
        ...m,
        result: t('profile.lose'),
        opponent: m.winnerLabel
    }));

    const MyMatches = [...matchWins, ...matchLosses].sort((a, b) => 
                            ...
                            <ul className="search_val_box">
                                {getSearchVal.map((user) =>
                                    <li 
                                        ...
                                        onClick={async () => {
                                            setSearchVal('');
                                            const result = await axios.get(`/api/users/${user.id}`, {withCredentials:true})
