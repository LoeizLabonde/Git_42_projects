...
t_error	lexcat_redir_handler(t_block **head)
{
	...
	while (nav)
	{
		if (nav->type == REDIR_I && nav->next)
			nav->next->type = INFILE;
		if (nav->type == REDIR_O && nav->next)
			nav->next->type = OUTFILE;
		if (nav->type == APPEND && nav->next)
			nav->next->type = OUTFILE;
		if (nav->type == HEREDOC && nav->next)
			nav->next->type = EOFHD;
		nav = nav->next;
	}
	return (NO_ERROR);
...
t_error	splitter(t_mo_shell *mo_shell)
{
	...
	if (look_for_redir(&mo_shell->splitted_input) == true)
		if (syntax_check_handler(mo_shell, &error_ret, &sch_ret))
			return (sch_ret);
	if (real_splitter(&mo_shell->splitted_input) == ERROR)
		return (mo_shell->les = 2, ERROR);
