...
static	t_error	update_env_var(char *arg, char *var_name, t_mo_shell *shell)
{
	...
	tmp2 = append(tmp2, "=", 1);
	if (var_exst(tmp2, shell->shell_env) == -1)
	{
...
static int	chck_vld_id(char *str)
{
	int	i;
...
int	ms_export(char **args, t_mo_shell *mo_shell, t_cmd *cmd)
{
	...
	((void)cmd, ret = 0, ar_i = 0);
	if (args[1])
		ret = ms_export_check_options(args, &ar_i);
	if (ret)
		return (ret);
	while (args[++ar_i] != NULL)
	{
		(ft_bzero(var_name, DF_BF_SZ), i = -1, ret = chck_vld_id(args[ar_i]));
		if (!ft_strchr(args[ar_i], '='))
			continue ;
		if (!ret)
		{
			while (args[ar_i][++i])
				if (iterate_through_str(args, &i, ar_i, var_name))
					break ;
			if (update_env_var(args[ar_i], var_name, mo_shell) == ERROR)
				return (-1);
		}
	}
	return (ret);
