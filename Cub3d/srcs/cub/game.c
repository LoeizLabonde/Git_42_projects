...
/*   Updated: 2025/03/11 16:57:06 by kduroux          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_memcpy(data->img_tab[IMG].addr, data->img_tab[BG].addr,
		WINDOWS_HEIGHT * WINDOWS_WIDTH * (data->img_tab[BG].bits_pp / 8));
}
...
int	ft_game_loop(t_game *game)
{
	...
	mlx_put_image_to_window(game->mlx, game->win, game->img_tab[IMG].ptr, 0, 0);
	return (0);
}
